...
set_property -dict [list \
    ...
    CONFIG.PSU__SAXIGP0__DATA_WIDTH {32}] [get_bd_cells ${ps_name}]
set_property -dict [list \
    CONFIG.PSU__MAXIGP0__DATA_WIDTH {32} \
    CONFIG.PSU__MAXIGP1__DATA_WIDTH {32}] [get_bd_cells ${ps_name}]
...
apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config " \
    ...
    Clk_slave {Auto} \
    Clk_xbar {Auto} \
    Master /${ps_name}/M_AXI_HPM0_FPD \
    Slave /${acc_name}/s_axi_CTRL_BUS \
    intc_ip {New AXI Interconnect} \
    master_apm {0}" [get_bd_intf_pins ${acc_name}/s_axi_CTRL_BUS]

apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config " \
   Clk_master /${ps_name}/pl_clk0 (100 MHz) \
   Clk_slave {Auto} \
   Clk_xbar {Auto} \
   Master /${acc_name}/m_axi_IN_BUS \
   Slave /${ps_name}/S_AXI_HPC0_FPD \
   intc_ip {Auto} \
   master_apm {0}" [get_bd_intf_pins ${ps_name}/S_AXI_HPC0_FPD]

apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config " \
    Clk_master {Auto} \
    Clk_slave /${ps_name}/pl_clk0 (100 MHz) \
    Clk_xbar /${ps_name}/pl_clk0 (100 MHz) \
    Master /${ps_name}/M_AXI_HPM1_FPD \
    Slave /${acc_name}/s_axi_CTRL_BUS \
    intc_ip {/ps8_0_axi_periph} \
    master_apm {0}" [get_bd_intf_pins ${ps_name}/M_AXI_HPM1_FPD]
